...
export default async function AdminCustomersPage() {
  ...
  return <div className="adminContent"><div className="adminTitle"><div><h2>Khách hàng & CRM</h2><p>Lưu địa chỉ, lịch sử mua, điểm, hạng, thẻ chăm sóc và ghi chú.</p></div></div><section className="adminPanel"><h2>Cấu hình hạng và giảm giá</h2><p className="muted">Admin có thể đổi tên hạng, ngưỡng điểm và phần trăm; đơn mới sẽ tự áp dụng.</p><div className="tierEditGrid">{tiers.map((tier) => <TierForm key={tier.id} tier={tier} />)}</div></section><div className="customerCrmList">{customers.map((customer) => {
